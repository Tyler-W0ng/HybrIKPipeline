...
class HRNetSMPLCam(nn.Module):
    ...
    def forward(self, x, flip_test=False, **kwargs):
        ...
        R = output.rot_mats
        R_to_save = R.detach().cpu()

        rot_file_path = 'rotation_matrices_fencing.pt'
        if os.path.exists(rot_file_path):

            existing_rotations = torch.load(rot_file_path, weights_only=True)
            combined_rotations = torch.cat([existing_rotations, R_to_save], dim=0)
        else:
            combined_rotations = R_to_save

        torch.save(combined_rotations, rot_file_path)
