...
pose_input = torch.stack(pose_inputs).to(opt.gpu)
bboxes = torch.tensor(bboxs, dtype=torch.float32).to(opt.gpu)
img_centers = torch.from_numpy(np.array(img_centers)).float().to(opt.gpu)

batch_size = 32
all_outputs = []

for i in tqdm(range(0, len(pose_inputs), batch_size)):
    batch_pose_input = pose_input[i:i+batch_size]
    batch_bboxes = bboxes[i:i+batch_size]
    batch_img_centers = img_centers[i:i+batch_size]
...
#         bboxes=bboxes,
#         img_center=img_centers
#     )

#     uv_29 = pose_output.pred_uvd_jts.reshape(-1, 29, 3)[:, :, :2]  # shape: (B, 29, 2)
#     transl = pose_output.transl.detach()  # shape: (B, 3)
...
# pose_input = torch.stack(pose_inputs).to(opt.gpu)
# bboxes = torch.tensor(bboxs, dtype=torch.float32).to(opt.gpu)
# img_centers = torch.tensor(img_centers, dtype=torch.float32).to(opt.gpu)
...
# Trigger execution of offset file generating script
